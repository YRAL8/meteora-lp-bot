# TypeScript dry-run layer (фаза B2 / B2.1 / B2.2)

Сборка и симуляция транзакций Meteora DLMM через официальный SDK
`@meteora-ag/dlmm`. **Только сборка + `simulateTransaction`.** Подписи и
отправка в сеть запрещены (фаза B3).
